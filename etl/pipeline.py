...
def main():
    ...
    if files_to_extract == ["all"]:
        subprocess.run(["python", "/workspaces/synpuf-etl/etl/extract.py", "/workspaces/synpuf-etl/data/BASE_SYNPUF_INPUT_DIRECTORY", "all"], check=True)
        # wait for extract.py to finish before continuing
        while not os.path.exists(os.environ['BASE_SYNPUF_INPUT_DIRECTORY']):
            time.sleep(1)
        subprocess.run(["python", "/workspaces/synpuf-etl/etl/transform.py", "all"], check=True)
        ...
        for file_number in files_to_extract:
            ...
            subprocess.run(["python", "/workspaces/synpuf-etl/etl/extract.py", "/workspaces/synpuf-etl/data/BASE_SYNPUF_INPUT_DIRECTORY", file_number], check=True)
            while not os.path.exists(os.environ['BASE_SYNPUF_INPUT_DIRECTORY']):
                time.sleep(1)
            subprocess.run(["python", "/workspaces/synpuf-etl/etl/transform.py", file_number], check=True)

    # Check if the input is either "overwrite" or "append"
    if load_method == "overwrite" or load_method == "append":
        subprocess.run(["python", "/workspaces/synpuf-etl/etl/load.py", load_method])
    else:
        return
